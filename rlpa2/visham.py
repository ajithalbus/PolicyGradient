...
class visham(Env):
    ...
    def best_move(self,action):
        return action/np.linalg.norm(action)*0.025

    def _step(self, action):
        #print action
        done = False
        #print action
        action = self.best_move(action)
        #print action
        self.state += action
        reward = -np.linalg.norm(self.state)
        if not self.observation_space.contains(self.state+action) or (self.state[0]**2+self.state[1]**2)<1e-2:
            done=True
        

        reward=-np.sqrt((0.5*(self.state[0]**2) + 0.5*10*(self.state[1]**2)))
        #Fill your code here
        return self.state,reward,done,{}# Return the next state and the reward, along with 2 additional quantities : False, {}
